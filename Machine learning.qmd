...
expr_df_matched <- readRDS("expr_df_matched.rds")
pheno_clean <- readRDS("pheno_clean.rds")
common_genes_data <- read.csv("Common_Significant_Genes.csv")
common_genes <- common_genes_data$Gene

#Screen for common significant genes and transpose them
expr_common <- expr_df_matched[rownames(expr_df_matched) %in% common_genes, ]
...
na_count <- sum(is.na(expr_common_t))
cat("Number of NA values in expr_common_t:", na_count, "\n")

#Define target variables and fill in NA
threshold_sledai <- 10
median_sledai <- median(pheno_clean$sledai_at_baseline, na.rm = TRUE)
pheno_clean$sledai_at_baseline[is.na(pheno_clean$sledai_at_baseline)] <- median_sledai
pheno_clean$risk_group_SLEDAI <- ifelse(pheno_clean$sledai_at_baseline >= threshold_sledai, 
                                        "HighRisk_SLEDAI", 
                                        "LowRisk_SLEDAI")
target <- as.factor(pheno_clean$risk_group_SLEDAI)

#Check the NA in the target variable
na_target <- sum(is.na(target))
cat("Number of NA values in target:", na_target, "\n")

#If there is still NA, print the problem sample
if (na_target > 0) {
  na_samples <- which(is.na(target))
  cat("Samples with NA in target:", na_samples, "\n")
  stop("Target still contains NA values after imputation.")
}

#Align samples
common_samples <- intersect(rownames(expr_common_t), rownames(pheno_clean))
expr_common_t <- expr_common_t[common_samples, ]
pheno_clean <- pheno_clean[common_samples, ]
target <- as.factor(pheno_clean$risk_group_SLEDAI)

#Check the aligned dimensions
cat("Aligned expr_common_t rows:", nrow(expr_common_t), "\n")
cat("Aligned target length:", length(target), "\n")
...
set.seed(135)
train_idx <- sample(1:nrow(expr_common_t), 0.8 * nrow(expr_common_t))
train_data <- expr_common_t[train_idx, ]
test_data <- expr_common_t[-train_idx, ]
train_labels <- target[train_idx]
test_labels <- target[-train_idx]

#Check if the dimensions of the data and labels match
cat("Train data rows:", nrow(train_data), "Train labels length:", length(train_labels), "\n")
cat("Test data rows:", nrow(test_data), "Test labels length:", length(test_labels), "\n")

#Check the NA in the label
cat("NA in train_labels:", sum(is.na(train_labels)), "\n")
cat("NA in test_labels:", sum(is.na(test_labels)), "\n")

#If there is still NA, stop and prompt
if (sum(is.na(train_labels)) > 0 || sum(is.na(test_labels)) > 0) {
  stop("Train or test labels still contain NA values.")
}
